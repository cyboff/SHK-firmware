...
void loop() {
  ...
  //if ( !dataSent && !lastSentTimeout) {  // send only every 5 sec
    ...
    //   Serial1.print(",");
    //   Serial1.print(positionValueDisp);
    //   Serial1.print(",");
    //   Serial1.print(positionValueAvgDisp);
    //   Serial1.print(",");
    //   // Serial1.print(risingEdgeTimeDisp);
    //   // Serial1.print(",");
    //   // Serial1.print(peakValueTimeDisp);
    //   // Serial1.print(",");
    //   // Serial1.print(fallingEdgeTimeDisp);
    //   // Serial1.print(",");
  ...
  if (!refreshMenuTimeout) {
   ...
   // button interrupt check
    ...
    switch (currentMenu) {
      ...
      break;
      case MENU_FILTERS: showFiltersMenu();
      break;
      case MENU_FILTER_POSITION: setFilterPosition();
      break;
      case MENU_FILTER_ON: setFilterOn();
      break;
      case MENU_FILTER_OFF: setFilterOff();
      break;
      case MENU_ANALOG: showAnalogMenu();
      break;
      case MENU_WINDOW_BEGIN: setWindowBegin();
      break;
      case MENU_WINDOW_END: setWindowEnd();
      break;
      case MENU_POSITION_MODE: setPositionMode();
      break;
      case MENU_ANALOG_OUT_MODE: setAnalogOutMode();
      break;
      case MENU_POSITION_OFFSET: setPositionOffset();
      break;
...
void showMainMenu(void) {
  ...
  if (currentMenuOption == 1) {
    ...
    // switch (positionMode) {
    //   case 0: displayPrint("Pos %3d%%", peakValueTimeDisp / 10);
    //   break;
    //   case 1: displayPrint("Pos %3d%%", risingEdgeTimeDisp / 10);
    //   break;
    //   case 2: displayPrint("Pos %3d%%", fallingEdgeTimeDisp / 10);
    //   break;
    //   case 3: displayPrint("Pos %3d%%", peakValueTimeDisp / 10);
    //   break;
    //   default:
    //   break;
    // }
  }
  if (currentMenuOption == 2) displayPrint("Gain %2dx", pga);
  if (currentMenuOption == 3) displayPrint("Thre %2d%%", thre);
...
void showSetupMenu(void) {
  ...
  if (currentMenuOption == 2) displayPrint("Filters ");
  if (currentMenuOption == 3) displayPrint("Analog  ");
  if (currentMenuOption == 4) displayPrint("Info    ");
  ...
  if (lastKey == BTN_A) {
    if (currentMenuOption < 5) currentMenuOption++;
    else currentMenuOption = 0;
  }

  if (lastKey == BTN_B) {
    if (currentMenuOption > 0) currentMenuOption--;
    else currentMenuOption = 5;
  }
...
void showModbusMenu(void) {
  ...
  if (lastKey == BTN_BH) { // ENTER
    ...
    if (currentMenuOption == 1) {
      ...
      for (int i = 0; i < 6; i++) { // find actual speed in array
        if (modbusSpeedArray[i]*100 == menu_modbusSpeed) actualSpeed = i;
      }
    }
    if (currentMenuOption == 2) {
...
void setModbusSpeed(void) {
  ...
  if (lastKey == BTN_A) {
    if (actualSpeed > 0) actualSpeed--; else actualSpeed = 6;
    menu_modbusSpeed = modbusSpeedArray[actualSpeed]*100;
  }
  if (lastKey == BTN_B) {  // increment by 1
    if (actualSpeed < 6 ) actualSpeed++; else actualSpeed = 0;
    menu_modbusSpeed = modbusSpeedArray[actualSpeed]*100;
  }
  if (lastKey == BTN_AH) { // ESC
    currentMenu = MENU_MODBUS;
...
void showFiltersMenu(void) {
  ...
  if (currentMenuOption == 0) displayPrint("fPos%4d", filterPosition);
  if (currentMenuOption == 1) displayPrint("fOn %4d", filterOn);
  if (currentMenuOption == 2) displayPrint("fOff%4d", filterOff);
...
void setFilterPosition(void) {
  ...
  if (blinkMenu) displayPrint("fPos%4d", menu_filterPosition); else displayPrint("    %4d", menu_filterPosition);

  if (lastKey == BTN_BH) { // increment by 1,10,100
    switch (incID) {
      case 1:   incID = 10; displayPrint("fPos x10"); delay(500);
      break;
      case 10: incID = 100; displayPrint("fPosx100"); delay(500);
      break;
      case 100:  incID = 1; displayPrint("fPos  x1"); delay(500);
      break;
  ...
  //check range in ms
  if (menu_filterPosition < 0) menu_filterPosition = 9999;
  if (menu_filterPosition > 9999) menu_filterPosition = 0;
...
void setFilterOn(void) {
  ...
  if (blinkMenu) displayPrint("fOn %4d", menu_filterOn); else displayPrint("    %4d", menu_filterOn);

  if (lastKey == BTN_BH) { // increment by 1,10,100
    switch (incID) {
      case 1:   incID = 10; displayPrint("fOn  x10"); delay(500);
      break;
      case 10: incID = 100; displayPrint("fOn x100"); delay(500);
      break;
      case 100:  incID = 1; displayPrint("fOn   x1"); delay(500);
      break;
  ...
  //check range in ms
  if (menu_filterOn < 0) menu_filterOn = 9999;
  if (menu_filterOn > 9999) menu_filterOn = 0;
...
void setFilterOff(void) {
  ...
  if (blinkMenu) displayPrint("fOff%4d", menu_filterOff); else displayPrint("    %4d", menu_filterOff);

  if (lastKey == BTN_BH) { // increment by 1,10,100
    switch (incID) {
      case 1:   incID = 10; displayPrint("fOff x10"); delay(500);
      break;
      case 10: incID = 100; displayPrint("fOffx100"); delay(500);
      break;
      case 100:  incID = 1; displayPrint("fOff  x1"); delay(500);
      break;
  ...
  //check range in ms
  if (menu_filterOff < 0) menu_filterOff = 9999;
  if (menu_filterOff > 9999) menu_filterOff = 0;
...
void setWindowBegin(void) {
  ...
  if (blinkMenu) displayPrint("wBeg%3d%%", menu_windowBegin); else displayPrint("    %3d%%", menu_windowBegin);
  ...
  if (menu_windowBegin < 5) menu_windowBegin = 50;
  if (menu_windowBegin > 50) menu_windowBegin = 5;
...
void setWindowEnd(void) {
  ...
  if (blinkMenu) displayPrint("wEnd%3d%%", menu_windowEnd); else displayPrint("    %3d%%", menu_windowEnd);
  ...
  if (menu_windowEnd < 50) menu_windowEnd = 95;
  if (menu_windowEnd > 95) menu_windowEnd = 50;
...
void setPositionMode(void) {
  ...
  if (blinkMenu) displayPrint("mPos%s", menu_positionModeDisp[menu_positionMode]); else displayPrint("    %s", menu_positionModeDisp[menu_positionMode]);

  if (lastKey == BTN_A) {  // increment by 1
    if (menu_positionMode < 3 ) menu_positionMode++; else menu_positionMode = 0;
  }
  if (lastKey == BTN_B) {
    if (menu_positionMode > 0) menu_positionMode--; else menu_positionMode = 3;
  }
...
void setAnalogOutMode(void) {
  ...
  if (lastKey == BTN_B) {
    if (menu_analogOutMode > 0) menu_analogOutMode--; else menu_analogOutMode = 3;
  }
...
void setPositionOffset(void) {
  ...
  if (blinkMenu) displayPrint("Offs%3d%%", menu_positionOffset); else displayPrint("    %3d%%", menu_positionOffset);
  ...
  if (menu_positionOffset < 5) menu_positionOffset = 95;
  if (menu_positionOffset > 95) menu_positionOffset = 5;
...
void showInfoMenu(void) {
  ...
  if (currentMenuOption == 1) displayPrint("SN %5d", MODEL_SERIAL_NUMBER);
  if (currentMenuOption == 2) displayPrint("FW %5d", FW_VERSION);
  if (currentMenuOption == 3) displayPrint("Temp %2dC", celsius);
  if (currentMenuOption == 4) displayPrint("MaxT %2dC", max_temperature); 
  if (currentMenuOption == 5) displayPrint("TT%6d", total_runtime);
  if (currentMenuOption == 6) displayPrint("FacReset");


  if (lastKey == BTN_A) {
    if (currentMenuOption < 6) currentMenuOption++;
    else currentMenuOption = 0;
  }

  if (lastKey == BTN_B) {
    if (currentMenuOption > 0) currentMenuOption--;
    else currentMenuOption = 6;
  }
...
void config_writeDefaultsToEEPROM() { // flash is empty or program version changed
    // writes sign codes
...
void reset_writeDefaultsToEEPROM() {  // reset all values (except calibration values) to fact. defaults
    // writes sign codes
    // eeprom_writeInt(EE_ADDR_MODEL_TYPE, MODEL_TYPE);
    // eeprom_writeInt(EE_ADDR_MODEL_SERIAL_NUMBER, MODEL_SERIAL_NUMBER);
    // eeprom_writeInt(EE_ADDR_FW_VERSION, FW_VERSION);
...
void checkALARM() {
  ...
  // check runtime
  if (!hourTimeout) { // every hour
    hourTimeout = 3600000;
    total_runtime++;
    if ((total_runtime%4) == 1) { // every 4 hours
      eeprom_writeInt(EE_ADDR_total_runtime, total_runtime);
    }
  }
...
void timer500us_isr(void) {   //every 500us
  ...
  if (!digitalReadFast(MOTOR_CLK)) hourTimeout--;  // every 1ms
...
void adc0_isr(void) {
  ...
  if ((analogBufferIndex > windowBegin*2) && (analogBufferIndex < windowEnd*2)) {  // from % to index of 0-200
    ...
    if ( (peakValue > thre256 + hmdThresholdHyst) || ( digitalReadFast(LED_SIGNAL) && (peakValue > thre256 - hmdThresholdHyst) )
      ...
      if (!risingEdgeTime) { // only first occurence
  ...
  if (analogBufferIndex < ANALOG_BUFFER_SIZE-1) {  // some delay for update results
    analogBufferIndex++;
  }
  else {
    // update outputs
...
void updateResults() {
  ...
  switch (positionMode) {  // for display
    case 1: positionValueDisp = risingEdgeTime;
     break;
    case 2: positionValueDisp = fallingEdgeTime;
     break;
    case 3:
    case 0: positionValueDisp = peakValueTime;
     break;
  }
  ...
  // update Modbus registers before remaping 
  // holdingRegs[PEAK_VALUE] = peakValueDisp;
  // holdingRegs[POSITION_VALUE] = positionValue; // for signal graph in range of wBeg - wEnd
  // holdingRegs[POSITION_VALUE_AVG] = positionValueAvgDisp; // AVG position 0 - 1000 
  ...
  switch (analogOutMode) { //an1/an2: "1Int2Pos" = 0, "1Pos2Int" = 1, "1Int2Int" = 2, "1Pos2Pos" = 3
    case 0: updateSPI((peakValueDisp<<8), positionValue); // range is 2x 16bit
     break;
    case 1: updateSPI(positionValue, (peakValueDisp<<8));
     break;
...
void checkModbus() {
  ...
  if ((holdingRegs[MODBUS_ID] != modbusID) || (holdingRegs[MODBUS_SPEED]*100 != modbusSpeed) || (holdingRegs[MODBUS_FORMAT] != modbusFormat)) {
    if (holdingRegs[MODBUS_ID] != modbusID && holdingRegs[MODBUS_ID] > 0 && holdingRegs[MODBUS_ID] < 248 ) {
      modbusID = holdingRegs[MODBUS_ID]; eeprom_writeInt(EE_ADDR_modbus_ID, modbusID);
    }
    ...
    if (holdingRegs[MODBUS_SPEED]*100 != modbusSpeed) {
      switch (holdingRegs[MODBUS_SPEED]*100) {
        ...
        case 38400:
        case 57600:
        case 115200: modbusSpeed = holdingRegs[MODBUS_SPEED]*100; eeprom_writeInt(EE_ADDR_modbus_Speed, modbusSpeed/100);
         break;
        default:
         break;
      }
    ...
    if (holdingRegs[MODBUS_FORMAT] != modbusFormat) {
      switch (holdingRegs[MODBUS_FORMAT]) {
        ...
        case SERIAL_8E1:
        case SERIAL_8O1:
        case SERIAL_8N2: modbusFormat = holdingRegs[MODBUS_FORMAT]; eeprom_writeInt(EE_ADDR_modbus_Format, modbusFormat);
         break;
        default:
         break;
      }
  ...
  if (holdingRegs[SET] != set && holdingRegs[SET] < 3) { // RELAY = 0 (REL1 || REL2), MAN1 = 1, MAN2 = 2
    set = holdingRegs[SET]; eeprom_writeInt(EE_ADDR_set, set); 
  }

  if (holdingRegs[GAIN_SET1] != pga1) {
    switch (holdingRegs[GAIN_SET1]) { // check for valid values
      case 1:
      case 2:
      case 4:
      case 8:
      case 16:
      case 32:
      case 64: pga1 = holdingRegs[GAIN_SET1]; eeprom_writeInt(EE_ADDR_gain_set1, pga1); 
        break;
      default:
        break;
    }
  ...
  if (holdingRegs[THRESHOLD_SET1] != thre1 && holdingRegs[THRESHOLD_SET1] >= 20 && holdingRegs[THRESHOLD_SET1] <= 80) {
    thre1 = holdingRegs[THRESHOLD_SET1]; eeprom_writeInt(EE_ADDR_threshold_set1, thre1);
  }
  ...
  if (holdingRegs[GAIN_SET2] != pga2) {
    switch (holdingRegs[GAIN_SET2]) {
      ...
      case 16:
      case 32:
      case 64: pga2 = holdingRegs[GAIN_SET2]; eeprom_writeInt(EE_ADDR_gain_set2, pga2);
        break;
      default:
        break;
    }  
  ...
  if (holdingRegs[THRESHOLD_SET2] != thre2 && holdingRegs[THRESHOLD_SET2] >= 20 && holdingRegs[THRESHOLD_SET2] <= 80) {
    thre2 = holdingRegs[THRESHOLD_SET2]; eeprom_writeInt(EE_ADDR_threshold_set2, thre2);
  }

  if (holdingRegs[WINDOW_BEGIN] != windowBegin && holdingRegs[WINDOW_BEGIN] >= 5 && holdingRegs[WINDOW_BEGIN] <= 50) {
    windowBegin = holdingRegs[WINDOW_BEGIN]; eeprom_writeInt(EE_ADDR_window_begin, windowBegin);
  }

  if (holdingRegs[WINDOW_END] != windowEnd && holdingRegs[WINDOW_END] >= 50 && holdingRegs[WINDOW_END] <= 95) {
    windowEnd = holdingRegs[WINDOW_END]; eeprom_writeInt(EE_ADDR_window_end, windowEnd);
  }

  if (holdingRegs[POSITION_MODE] != positionMode && holdingRegs[POSITION_MODE] < 4) {
    positionMode = holdingRegs[POSITION_MODE]; eeprom_writeInt(EE_ADDR_position_mode, positionMode);
  }

  if (holdingRegs[ANALOG_OUT_MODE] != analogOutMode && holdingRegs[ANALOG_OUT_MODE] < 4) {
    analogOutMode = holdingRegs[ANALOG_OUT_MODE]; eeprom_writeInt(EE_ADDR_analog_out_mode, analogOutMode);
  }

  if (holdingRegs[POSITION_OFFSET] != positionOffset && holdingRegs[POSITION_OFFSET] >= 5 && holdingRegs[POSITION_OFFSET] <= 95) {
    positionOffset = holdingRegs[POSITION_OFFSET]; eeprom_writeInt(EE_ADDR_position_offset, positionOffset);
  }

  if (holdingRegs[FILTER_POSITION] != filterPosition && holdingRegs[FILTER_POSITION] < 10000) {
    filterPosition = holdingRegs[FILTER_POSITION]; eeprom_writeInt(EE_ADDR_filter_position, filterPosition);
  }

  if (holdingRegs[FILTER_ON] != filterOn && holdingRegs[FILTER_ON] < 10000) {
    filterOn = holdingRegs[FILTER_ON]; eeprom_writeInt(EE_ADDR_filter_on, filterOn);
  }

  if (holdingRegs[FILTER_OFF] != filterOff && holdingRegs[FILTER_OFF] < 10000) {
    filterOff = holdingRegs[FILTER_OFF]; eeprom_writeInt(EE_ADDR_filter_off, filterOff);
  }
  
  if (holdingRegs[IO_STATE] != io_state) {
    if (holdingRegs[IO_STATE] & (1<<IO_LASER)) { // check if IO_LASER bit is set
      laserTimeout = TIMEOUT_LASER;
    ...
    if (holdingRegs[IO_STATE] & (1<<IO_IR_LED)) { // check if IO_IR_LED bit is set
      digitalWrite(IR_LED, HIGH);
      testTimeout = TIMEOUT_TEST;
      intTest = true;
    } else {
